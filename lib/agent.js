...
Agent.prototype.timeout = function(ms) {
  this.maxTimeout = ms
  return this
}
...
Agent.prototype.send = function(req, cb) {
  ...
  util.befores(req, this.befores, 0, function(err) {
    ...
    timer = self.maxTimeout && setTimeout(function() {
      timer = null
      aborted = true
      var err = new Error('Timeout of ' + self.maxTimeout + 'ms exceeded.')
      if (body) {
        // We received headers and have a body stream
        // push the error to it
        body.push(err)
      } else {
        // No response yet and no work for middlewares
        // yield directly
        done(err)
      }
      state.req.abort()
    ...
    send(req, state, function(err, res) {
      if (aborted) return // we got ECONNRESET due to state.req.abort(), ignore
      if (received) {
        // sheet happened, warn about it
...
function send(req, state, cb) {
  ...
  var nativeReq = state.req = mod.request(opts)
  .on('error', cb)
  .on('response', function(res) {
    ...
    if (this.maxRedirects === false || util.isNotRedirect(res.statusCode))
      return cb(null, new Response(res.statusCode, res.headers, Simple(res)))
